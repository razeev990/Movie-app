...
export default function App() {
  ...
  const openMovieDetails = async (movie) => {
    ...
    try {
      ...
      const officialTrailer = vidJson.results
        ? vidJson.results.find(
      ...
      const regionData = provJson.results
        ? provJson.results.IN || provJson.results.US
        : null;
      setProviders(
        regionData ? regionData.flatrate || regionData.buy || [] : []
      );
          ...
          <ScrollView>
            ...
            <View style={styles.modalBody}>
              ...
              <Text style={styles.modalSubtitle}>
                Rating: {selectedMovie && selectedMovie.vote_average ? selectedMovie.vote_average.toFixed(1) : 'N/A'}/10 | Release: {selectedMovie ? selectedMovie.release_date : ''}
              </Text>
...
const styles = StyleSheet.create({
  ...
  castCard: { width: export default function App() {
  const [movies, setMovies] = useState([]);
  const [loading, setLoading] = useState(true);
  const [tab, setTab] = useState('popular');
  const [selectedGenre, setSelectedGenre] = useState('all');
